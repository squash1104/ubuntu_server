...
def is_recepcionista(user):
    return user.is_authenticated and user.groups.filter(name__in=["Recepcionista", "Supervisor", "Admin"]).exists()


def is_atendente(user):
    return user.is_authenticated and user.groups.filter(name__in=["Atendente", "Supervisor", "Admin"]).exists()


def is_supervisor(user):
    return user.is_authenticated and user.groups.filter(name__in=["Supervisor", "Admin"]).exists()
...
def home(request):
    ...
    try:
        ...
        if preset:
            if preset == "today":
                ...
                first = Atendimento.objects.order_by("horario_chegada").values_list("horario_chegada", flat=True).first()
                if first:
                    dt_ini = first.date()
                    dt_fim = hoje
                else:
                ...
                dt_fim = timezone.datetime.fromisoformat(dt_fim_str).date() if dt_fim_str else hoje
                dt_ini = timezone.datetime.fromisoformat(dt_ini_str).date() if dt_ini_str else (hoje - timedelta(days=29))
        else:
            dt_fim = timezone.datetime.fromisoformat(dt_fim_str).date() if dt_fim_str else hoje
            dt_ini = timezone.datetime.fromisoformat(dt_ini_str).date() if dt_ini_str else (hoje - timedelta(days=29))
    except Exception:
    ...
    base_qs = Atendimento.objects.filter(horario_chegada__date__gte=dt_ini, horario_chegada__date__lte=dt_fim)

    total_atend = base_qs.filter(status=AtendimentoStatus.CONCLUIDO).count()  # Apenas concluídos
    ...
    # Médias de tempo
    espera_expr = ExpressionWrapper(F("inicio_atendimento") - F("horario_chegada"), output_field=DurationField())
    atendimento_expr = ExpressionWrapper(F("fim_atendimento") - F("inicio_atendimento"), output_field=DurationField())

    avg_espera = base_qs.filter(inicio_atendimento__isnull=False).aggregate(v=Avg(espera_expr))["v"]
    avg_atend = base_qs.filter(fim_atendimento__isnull=False, inicio_atendimento__isnull=False).aggregate(v=Avg(atendimento_expr))["v"]
    ...
    while dia <= dt_fim:
        ...
        serie_chegadas.append(Atendimento.objects.filter(horario_chegada__date=dia).count())
        serie_concluidos.append(Atendimento.objects.filter(status=AtendimentoStatus.CONCLUIDO, fim_atendimento__date=dia).count())
    ...
    atendentes_avg_at = [int(p["tempo_medio_at"].total_seconds() / 60) if p["tempo_medio_at"] else 0 for p in por_atendente]
    atendentes_avg_esp = [int(p["tempo_medio_esp"].total_seconds() / 60) if p["tempo_medio_esp"] else 0 for p in por_atendente]
    atendentes_rows = [
        {
    ...
    while cursor <= end_month:
        prox = (cursor.replace(day=28) + timedelta(days=4)).replace(day=1)  # primeiro dia do mês seguinte
        qtd = Atendimento.objects.filter(horario_chegada__date__gte=cursor, horario_chegada__date__lt=prox).count()
        esp = Atendimento.objects.filter(inicio_atendimento__isnull=False, horario_chegada__date__gte=cursor, horario_chegada__date__lt=prox).aggregate(v=Avg(espera_expr))["v"]
        atd = Atendimento.objects.filter(fim_atendimento__isnull=False, inicio_atendimento__isnull=False, inicio_atendimento__date__gte=cursor, inicio_atendimento__date__lt=prox).aggregate(v=Avg(atendimento_expr))["v"]
    ...
    for att in atendentes_stats:
        ...
        if att["tempo_total_atendimento"]:
            total_seconds = int(att["tempo_total_atendimento"].total_seconds() * att["total_atendimentos"])
    ...
    for at in atendimentos_page:
        ...
        try:
            if at.fim_atendimento and at.horario_chegada:
                total_seconds = int((at.fim_atendimento - at.horario_chegada).total_seconds())
                hours = total_seconds // 3600
                minutes = (total_seconds % 3600) // 60
        ...
        atendimentos_recentes.append({
            ...
            "atendente_nome": getattr(at.atendente, "nome", "-") if at.atendente else "-",
    ...
    for municipio_info in municipios_data:
        ...
        try:
            cidade = Cidade.objects.filter(
                nome_cidade__iexact=municipio_nome
            ).first()
...
def dashboard(request):
    ...
    total_segundos = sum(
        (a.fim_atendimento - a.inicio_atendimento).total_seconds()
        for a in concluidos
    )
...
def visitante_enfileirar(request, pk: int):
    ...
    if request.method == "POST":
        ...
        AtendimentoEvento.objects.create(atendimento=at, tipo=TipoEvento.CHEGADA, usuario=request.user)
        ...
        try:
            ...
            emat = Atendimento.objects.filter(status=AtendimentoStatus.EM_ATENDIMENTO).count()
            event_uuid = str(uuid.uuid4())
            payload = {
                "type": "recepcao_update",
                "action": "chegada",
                "id": at.id,
                "chegada": at.horario_chegada.isoformat() if at.horario_chegada else None,
        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JsonResponse({"ok": True, "event_uuid": event_uuid, **{k:v for k,v in payload.items() if k != 'type'}})
        # Notificação
...
def visitante_create(request):
    if request.method == "POST":
        ...
        if not nome:
            ...
            return render(request, "recepcao/visitante_form.html", {"error": "Informe o nome.", "cidades_mt": cidades_mt})
    ...
    cidades_mt = Cidade.objects.filter(uf_cidade__iexact="MT").order_by("nome_cidade")
    atendentes = Attendente.objects.all().order_by("nome")
    return render(request, "recepcao/visitante_form.html", {"cidades_mt": cidades_mt, "atendentes": atendentes})
...
def visitante_detail(request, pk: int):
    ...
    cidades_mt = Cidade.objects.filter(uf_cidade__iexact="MT").order_by("nome_cidade")
    atendentes = Attendente.objects.all().order_by("nome")
    return render(request, "recepcao/visitante_detail.html", {"visitante": visitante, "atendimentos": atendimentos, "cidades_mt": cidades_mt, "atendentes": atendentes})
...
def visitante_update(request, pk: int):
    ...
    cidades_mt = Cidade.objects.filter(uf_cidade__iexact="MT").order_by("nome_cidade")
    atendentes = Attendente.objects.all().order_by("nome")
    return render(request, "recepcao/visitante_form.html", {"visitante": visitante, "view_mode": view_mode, "cidades_mt": cidades_mt, "atendentes": atendentes})
...
def chamar_proximo(request):
    ...
    prox = Atendimento.objects.filter(status=AtendimentoStatus.AGUARDANDO).order_by("horario_chegada").first()
    if not prox:
    ...
    def _create_event_if_not_recent(at: Atendimento, tipo: str, usuario, window_seconds=5):
        last = at.eventos.order_by('-timestamp').first()
        if last and last.tipo == tipo and (dj_timezone.now() - last.timestamp).total_seconds() < window_seconds:
            return None
        return AtendimentoEvento.objects.create(atendimento=at, tipo=tipo, usuario=usuario)
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        atendente_name = None
        try:
            atendente_name = getattr(prox.atendente, "nome", None) or getattr(prox.atendente, "get_full_name", lambda: str(prox.atendente))()
        except Exception:
        ...
        payload = {
            ...
            "action": "iniciar",
            "id": prox.id,
            "inicio": prox.inicio_atendimento.isoformat() if prox.inicio_atendimento else None,
...
def atendimento_detail(request, pk: int):
    ...
    return render(request, "recepcao/atendimento_detail.html", {"atendimento": atendimento})
...
def atendimento_iniciar(request, pk: int):
    ...
    if atendimento.status == AtendimentoStatus.AGUARDANDO:
        ...
        if not (last and last.tipo == TipoEvento.INICIO and (dj_timezone.now() - last.timestamp).total_seconds() < 5):
            AtendimentoEvento.objects.create(atendimento=atendimento, tipo=TipoEvento.INICIO, usuario=request.user)
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        atendente_name = None
        try:
            atendente_name = getattr(atendimento.atendente, "nome", None) or getattr(atendimento.atendente, "get_full_name", lambda: str(atendimento.atendente))()
        except Exception:
...
def atendimento_encerrar(request, pk: int):
    ...
    if atendimento.status == AtendimentoStatus.EM_ATENDIMENTO:
        ...
        last = atendimento.eventos.order_by('-timestamp').first()
        if not (last and last.tipo == TipoEvento.FIM and (dj_timezone.now() - last.timestamp).total_seconds() < 5):
            AtendimentoEvento.objects.create(atendimento=atendimento, tipo=TipoEvento.FIM, usuario=request.user)
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        agu = Atendimento.objects.filter(status=AtendimentoStatus.AGUARDANDO).count()
        emat = Atendimento.objects.filter(status=AtendimentoStatus.EM_ATENDIMENTO).count()
        fim_iso = atendimento.fim_atendimento.isoformat() if atendimento.fim_atendimento else None
        event_uuid = str(uuid.uuid4())
        # calcula total_display (fim - chegada)
        total_display = "-"
        try:
            if atendimento.fim_atendimento and atendimento.horario_chegada:
                total_seconds = int((atendimento.fim_atendimento - atendimento.horario_chegada).total_seconds())
                hours = total_seconds // 3600
                minutes = (total_seconds % 3600) // 60
        ...
        payload = {
            ...
            "action": "encerrar",
            "id": atendimento.id,
            "chegada": atendimento.horario_chegada.isoformat() if atendimento.horario_chegada else None,
            "inicio": atendimento.inicio_atendimento.isoformat() if atendimento.inicio_atendimento else None,
            "fim": fim_iso,
            "visitante": atendimento.visitante.nome,
...
def atendimento_cancelar(request, pk: int):
    ...
    last = atendimento.eventos.order_by('-timestamp').first()
    if not (last and last.tipo == TipoEvento.CANCELAMENTO and (dj_timezone.now() - last.timestamp).total_seconds() < 5):
        AtendimentoEvento.objects.create(atendimento=atendimento, tipo=TipoEvento.CANCELAMENTO, usuario=request.user)
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        agu = Atendimento.objects.filter(status=AtendimentoStatus.AGUARDANDO).count()
        emat = Atendimento.objects.filter(status=AtendimentoStatus.EM_ATENDIMENTO).count()
        ...
        payload = {
            ...
            "chegada": atendimento.horario_chegada.isoformat() if atendimento.horario_chegada else None,
            "inicio": atendimento.inicio_atendimento.isoformat() if atendimento.inicio_atendimento else None,
            "fim": atendimento.fim_atendimento.isoformat() if atendimento.fim_atendimento else None,
            "visitante": atendimento.visitante.nome,
            "atendente": getattr(atendimento.atendente, "nome", None),
...
def atendimento_anexar(request, pk: int):
    ...
    if request.method == "POST" and request.FILES.get("arquivo"):
        ...
        AtendimentoEvento.objects.create(atendimento=atendimento, tipo=TipoEvento.ANEXO, usuario=request.user)
        return redirect("recepcao:atendimento_detail", pk=pk)
    return render(request, "recepcao/atendimento_anexos.html", {"atendimento": atendimento})
...
def declaracao_visitante(request, pk: int):
    ...
    return render(request, "recepcao/declaracao_visitante.html", {"visitante": visitante, "agora": timezone.now()})
