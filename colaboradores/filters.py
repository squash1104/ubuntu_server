...
from .models import Colaborador, Cidade, Bairro
...
class ColaboradorFilter(django_filters.FilterSet):
    ...
    data_cadastro__gte = django_filters.DateFilter(field_name='data_cadastro', lookup_expr='gte', label='Data de Cadastro (início)', widget=forms.DateInput(attrs={'type': 'date'}))
    data_cadastro__lte = django_filters.DateFilter(field_name='data_cadastro', lookup_expr='lte', label='Data de Cadastro (fim)', widget=forms.DateInput(attrs={'type': 'date'}))

    # Filtros de ForeignKey (Cidade e Bairro)
    cidade = django_filters.ModelChoiceFilter(queryset=Cidade.objects.all(),label='Cidade')
    bairro = django_filters.ModelChoiceFilter(queryset=Bairro.objects.all(),label='Bairro')
    ...
    class Meta:
        model = Colaborador
        fields = ['nome', 'cidade', 'bairro', 'data_cadastro__gte', 'data_cadastro__lte']