...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--dry-run", action="store_true", help="Apenas exibe o que seria alterado")
        parser.add_argument("--only-empty", action="store_true", help="Apenas bairros sem lat/lon")
    ...
    def handle(self, *args, **options):
        ...
        try:
            cidade = Cidade.objects.get(nome_cidade__iexact=nome_cidade, uf_cidade__iexact=uf)
        except Cidade.DoesNotExist:
            self.stdout.write(self.style.ERROR(f"Cidade não encontrada: {nome_cidade}-{uf}"))
            return

        qs = Bairro.objects.filter(cidade=cidade)
        if only_empty:
            qs = qs.filter(latitude_bairro__isnull=True) | qs.filter(longitude_bairro__isnull=True)
        ...
        for bairro in qs.order_by("nome_bairro"):
            result = geocode_bairro(bairro.nome_bairro, cidade=cidade.nome_cidade, uf=cidade.uf_cidade)
            if not result:
                self.stdout.write(self.style.ERROR(f"❌ {bairro.nome_bairro}: não encontrado"))
                falhas += 1
                continue
        ...
        self.stdout.write(self.style.SUCCESS(f"Concluído. Atualizados: {atualizados} | Falhas: {falhas}"))
