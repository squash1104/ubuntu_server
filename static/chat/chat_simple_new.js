...
// Funcao para mostrar/esconder dock de contatos
window.showContactsDock = function() {
    ...
    if (dock) {
        ...
        console.log('Dock nao encontrado');
    }
};
...
// Funcao para criar janela de chat
window.createChatWindow = function(username, fullName, isOnline = true) {
    ...
    if (!container) {
        console.log('Container de chat nao encontrado');
        return null;
    }
    ...
    chatWindow.innerHTML = 
        '<div class="chat-header">' +
            ...
            '<div class="chat-actions">' +
                '<button class="chat-action-btn" onclick="minimizeChat(this.closest(\'.chat-window\'))" title="Minimizar">-</button>' +
                '<button class="chat-action-btn" onclick="closeChat(\'' + username + '\')" title="Fechar">x</button>' +
            '</div>' +
        '</div>' +
        ...
        '<div class="chat-input">' +
            '<input type="text" class="chat-input-field" placeholder="Digite sua mensagem..." id="input-' + username + '">' +
            '<button class="chat-send-btn" onclick="sendChatMessage(\'' + username + '\')" title="Enviar">></button>' +
        '</div>';
...
// Funcao para carregar contatos (simulada)
window.loadContacts = function() {
    ...
    if (contactsList) {
        ...
        const mockContacts = [
            { username: 'usuario1', full_name: 'Joao Silva', is_online: true },
        ...
        mockContacts.forEach(function(contact) {
            ...
            li.innerHTML = 
                '<div style="display: flex; align-items: center; gap: 10px; padding: 8px;">' +
                    ...
                    '<button onclick="createChatWindow(\'' + contact.username + '\', \'' + contact.full_name + '\', ' + contact.is_online + ')" style="margin-left: auto; background: #007bff; color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer;">Chat</button>' +
                '</div>';
...
// Inicializacao quando o DOM estiver pronto
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM carregado - inicializando chat simples');
    
    // Event listener para o botao de chat
    const chatToggle = document.getElementById('chat-toggle');
    if (chatToggle) {
        console.log('Botao chat encontrado');
        chatToggle.onclick = function() {
            console.log('Botao chat clicado');
            const dock = document.getElementById('contacts-dock');
        ...
        console.log('Botao chat nao encontrado');
    }
    ...
    if (closeContacts) {
        ...
        console.log('Botao fechar nao encontrado');
    }
