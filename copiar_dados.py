...
def copy_colaboradores():
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT column_name FROM information_schema.columns WHERE table_name = 'colaboradores'")
...
def copy_convidados():
    """Copia convidados"""
    with connections['default'].cursor() as cursor:
        cursor.execute("SELECT id, nome, telefone, data_nascimento, cidade_id, bairro_id, colaborador_id, data_cadastro, cadastrado_por_id FROM convidados")
...
def copy_historico():
    """Copia histórico"""
    with connections['default'].cursor() as cursor:
        cursor.execute("SELECT id, data_hora, acao, usuario_id, detalhes, ip_address FROM historico_historico")
