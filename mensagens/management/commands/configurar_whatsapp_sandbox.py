...
class Command(BaseCommand):
    ...
    def verificar_sandbox_status(self, account_sid, auth_token):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Erro na conexão: {response.status_code}")
                )
    ...
    def ativar_sandbox(self, account_sid, auth_token, codigo):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Erro ao ativar: {response.text}")
                )
    ...
    def testar_envio(self, account_sid, auth_token):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.stdout.write(
                    self.style.SUCCESS("✅ Mensagem WhatsApp enviada com sucesso!")
                )
                self.stdout.write(f"   Message SID: {result.get('sid')}")
            else:
                self.stdout.write(
                    self.style.ERROR(f"❌ Erro ao enviar: {response.text}")
                )
    ...
    def mostrar_instrucoes_ativacao(self):
        ...
        self.stdout.write(
            "6. Execute: python manage.py configurar_whatsapp_sandbox --codigo=SEU_CODIGO"
        )
        self.stdout.write(
