...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--business-id", type=str, help="Business Account ID (opcional)"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not phone_id or not access_token:
            self.stdout.write(
                self.style.ERROR("❌ Erro: Phone ID e Access Token são obrigatórios")
            )
            self.stdout.write("\n🔧 USO:")
        ...
        if not os.path.exists(config_file):
            self.stdout.write(
                self.style.ERROR(f"❌ Arquivo {config_file} não encontrado")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS("✅ Configurações do WhatsApp Cloud API atualizadas!")
        )
        ...
        if config_ok:
            ...
            self.stdout.write(
                "python manage.py testar_whatsapp_cloud_api --telefone=5565999616000"
            )
        else:
