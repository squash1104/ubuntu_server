...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--mensagem",
            type=str,
            default="🎉 Teste do WhatsApp Cloud API! 🎉",
            help="Mensagem para enviar",
        )
        parser.add_argument(
            "--verificar", action="store_true", help="Apenas verificar configuração"
        )
    ...
    def handle(self, *args, **options):
        ...
        if resultado["success"]:
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Erro ao enviar: {resultado.get('error')}")
            )
