...
def user_settings(request):
    ...
    colaboradores_cadastrados = Colaborador.objects.filter(cadastrado_por=request.user).count()
    convidados_cadastrados = Convidado.objects.filter(colaborador__cadastrado_por=request.user).count()
    ...
    def calcular_ranking_usuario():
        ...
        usuarios_stats = []
        for user in Colaborador.objects.values_list('cadastrado_por', flat=True).distinct():
            if user:
                colab_count = Colaborador.objects.filter(cadastrado_por=user).count()
                conv_count = Convidado.objects.filter(colaborador__cadastrado_por=user).count()
    ...
    def calcular_badges_usuario(total, colaboradores, convidados):
        ...
        if total >= 1000:
            badges.append({"emoji": "👑", "nome": "Rei dos Cadastros", "cor": "bg-danger"})
        elif total >= 500:
            ...
            badges.append({"emoji": "⭐", "nome": "Super Cadastrador", "cor": "bg-success"})
        elif total >= 25:
    ...
    badges_usuario = calcular_badges_usuario(total_cadastros_usuario, colaboradores_cadastrados, convidados_cadastrados)
    
    # Histórico do usuário (paginado)
    historico_usuario = Historico.objects.filter(usuario=request.user).order_by('-data_hora')
